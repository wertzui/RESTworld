...
public class TestService : CrudServiceBase<BlogDatabase, TestEntity, TestDto, TestDto, TestDto, TestDto>
{
    ...
    static TestService()
    {
        ...
        _fixture.Customize<TestEntity>(composer => composer
            ...
            .With(t => t.MyEnum, () => Enum.GetValues<TestEnum>().OrderBy(e => Random.Shared.Next()).First())
            .With(t => t.MyReadonlyEnum, () => Enum.GetValues<TestEnum>().OrderBy(e => Random.Shared.Next()).First())
            .With(t => t.MyFlagsEnum, () => Enum.GetValues<TestFlagsEnum>().Where(e => Random.Shared.NextDouble() >= 0.5).Aggregate((TestFlagsEnum)0, (e1, e2) => e1 | e2))
